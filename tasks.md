...
> **Revision (William, 2026-08-29):** `ListingForm.tsx` no longer silently uses `getCurrentPosition()` at submit time. Adds `LocationPicker.tsx` — a small draggable-pin map, seeded from the device's current position (one-shot), that the user confirms or adjusts before publishing; the submit handler now uses that confirmed point and blocks with a clear message if the picker hasn't resolved yet. Also extracted the Google Maps script-loading logic (injection + readiness polling) out of `MapView.tsx` into `/lib/google-maps-loader.ts`, shared by both, and switched `MapView.tsx`'s own position tracking from `watchPosition` to a one-shot `getCurrentPosition()` per map visit (see the FR-003 revision below for why). File scope: `/components/LocationPicker.tsx` (new), `/lib/google-maps-loader.ts` (new), `/components/ListingForm.tsx` (extend), `/components/MapView.tsx` (extend, do not rewrite).
> **Test command**: `pnpm test` (full suite — no dedicated test file for the picker itself, matching the existing convention of not unit-testing map/geolocation UI; verified manually in a real browser, including dragging the pin and confirming the stored listing coordinate matches the dragged position, not the raw device GPS reading)
...
> **Revision (William, 2026-08-29):** switched `MapView.tsx`'s own-position tracking from `navigator.geolocation.watchPosition` to a one-shot `getCurrentPosition()` on each map visit. `watchPosition` fires repeatedly from GPS jitter alone on real devices, cascading into a nearby-listings refetch and full marker rebuild every few seconds — traced as the root cause of the item-sheet-closes-mid-tap bug fixed earlier the same day. A single fetch per visit matches what the map already did on each tab visit (`active` toggling) and removes the churn. File scope: `/components/MapView.tsx` (extend).
...
> **Revision (William, 2026-08-29):** restyled `ChatWindow.tsx` and `/app/chat/[chatId]/page.tsx` (previously a plain `<ul>`/`<input>` with no design-system styling) — own vs. other message bubbles (right/accent vs. left/card), a header showing the other participant's name and avatar (fetched via the chat's two `user_id`s and `profiles`), the safety note restyled, auto-scroll to the newest message. No Figma reference this time; matched the established look from the map/board/listing-detail pages. `bottomRef.current?.scrollIntoView?.(...)` is guarded because jsdom (tests) doesn't implement it. File scope: `/components/ChatWindow.tsx` (rewrite), `/app/chat/[chatId]/page.tsx` (rewrite).
> **Test command**: `pnpm test -- chat-realtime.test.ts` (updated: the old assertion checked for literal "Them:" text, which the redesign replaced with bubble alignment/color — now asserts the received bubble's `<li>` has the left-aligned class)

> **Revision (William, 2026-08-29):** restyled again against a pasted screenshot + new `design.md` (thinner borders, flatter secondary surfaces, hard shadows reserved for emphasis, no Lucide icons/emojis introduced in this pass — kept the shared `BackButton` as-is since it's used elsewhere). Adds `chats.listing_id` (`supabase/migrations/*_chats_listing_id.sql`) so the chat header can show a real listing-summary card (photo/name/status/category) instead of a fabricated "Rating" trust card — set on chat creation via `?listingId=` on the listing detail page's Chat link, threaded through `/app/chat/new/page.tsx` and `/api/chat/open`, backfilled onto an existing chat if it started without one. File scope: `/components/ChatWindow.tsx` (extend), `/app/chat/[chatId]/page.tsx` (extend), `/app/chat/new/page.tsx` (extend), `/app/api/chat/open/route.ts` (extend), `/app/listings/[id]/page.tsx` (extend, one-line link change), `supabase/migrations/*_chats_listing_id.sql` (new).
> **Test command**: `pnpm test -- chat-create.test.ts` (added cases for storing `listing_id` on create and backfilling it on reopen)
